...
export async function buildWorkbook(run) {
  ...
  // Gap cells (a value the column otherwise has, but missing here) → amber, so
  // the empty cell to fill in is obvious in the data itself.
  const colIdx = (name) => columns.findIndex((c) => String(c).toLowerCase() === String(name).toLowerCase());
  for (const g of run.reconciliation?.gaps || []) {
    const ci = colIdx(g.column);
    if (ci !== -1) data.getCell(g.row + 2, ci + 1).fill = FILL_UNREAD;
  }

  // Totals that don't re-add → a bold red check-row under the data, with the
  // re-added figure placed under its own column so the discrepancy is in context.
  for (const t of run.reconciliation?.totals || []) {
    if (t.ok !== false) continue;
    const ci = colIdx(t.column);
    const note = new Array(columns.length).fill('');
    note[0] = `⚠ ${t.column}: re-added ${t.computed} ≠ printed ${t.printed}`;
    if (ci > 0) note[ci] = t.computed;
    const xr = data.addRow(note);
    for (let i = 1; i <= columns.length; i++) {
      const c = xr.getCell(i);
      c.fill = FILL_FLAG;
      c.font = { bold: true, color: { argb: 'FF8A2C2C' } };
    }
  }
