#!/usr/bin/env bash
#
# install.sh — one-command installer for the Extract PDF Node (macOS).
#
# A business runs ONE line in the built-in Terminal app — nothing to install by hand:
#
...
REPO="${GROUNDED_REPO:-pauldevelopai/node-bair-extract}"
REF="${GROUNDED_REF:-main}"
NODE_VERSION="${GROUNDED_NODE_VERSION:-20.18.1}"
GROUNDED_HOME="${GROUNDED_HOME:-$HOME/GROUNDED}"
APP_DIR="$GROUNDED_HOME/node-bair-extract"
DISPLAY_NAME="Extract PDF"
PORT="${PORT:-3000}"
