...
export async function extract(host, { buffer, rules, hasKey }) {
  ...
  const posture = cls.fileType === 'scan' ? 'high' : 'normal';
  ...
  return {
    ...
    flags,
    notes: !hasKey
      ? ['No AI key set — this is a plain code extraction. Add a key for AI structuring + the double-pass cross-check.']
      : (cls.fileType === 'scan' ? ['Scanned document — highest-risk path. Verify the flagged cells against the original.'] : []),
  };
}
