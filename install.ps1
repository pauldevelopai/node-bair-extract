# install.ps1 — one-command installer for the Extract PDF Node (Windows).
#
# A business runs ONE line in the built-in PowerShell — nothing to install by hand:
#
...
# business's API key (.env) and uploaded data (data\) are ALWAYS preserved.
#
# Everything below is for Develop AI, not the business — they only ever see the
# friendly messages this prints.

$ErrorActionPreference = 'Stop'

# ── Settings (defaults are the real values; env vars override them for testing) ──
$Repo    = if ($env:GROUNDED_REPO)         { $env:GROUNDED_REPO }         else { 'pauldevelopai/node-bair-extract' }
...
$AppDir  = Join-Path $Root 'node-bair-extract'
$Name    = 'Extract PDF'
...
if ($gotCode) {
  # Copy code in, but NEVER touch the business's key (.env) or data (data\).
